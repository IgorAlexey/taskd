...
`
	...
	var createdAt int64
	if err := db.ro.QueryRow("SELECT created_at FROM tasks WHERE body = 'old task'").Scan(&createdAt); err != nil {
	...
	if err := db.ro.QueryRow("SELECT created_at FROM tasks WHERE id = ?", created.ID).Scan(&postCreatedAt); err != nil {
