...
Append-only notes (`POST /tasks/{id}/notes`) record history, retrievable via
`GET /tasks/{id}/notes` or inline on `GET /tasks/{id}` without mutating the task specification.
